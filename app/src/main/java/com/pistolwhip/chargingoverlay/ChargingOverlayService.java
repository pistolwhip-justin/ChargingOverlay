...
public class ChargingOverlayService extends Service {
    ...
    @Override public void onCreate() {
        ...
        powerReceiver = new BroadcastReceiver() {
            @Override public void onReceive(Context context, Intent intent) { updateChargingState(); }
        };
    ...
    private void createDot() {
        ...
        try { windowManager.addView(dot, params); } catch (Exception ignored) { }
    }
    ...
    private void updateChargingState() {
        ...
        boolean nowCharging = status == BatteryManager.BATTERY_STATUS_CHARGING || status == BatteryManager.BATTERY_STATUS_FULL;
        charging = nowCharging;
        if (dot != null) dot.setVisibility(charging ? View.VISIBLE : View.GONE);
    }
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID, "ChargingOverlay", NotificationManager.IMPORTANCE_LOW);
    ...
    @Override public void onDestroy() {
        if (powerReceiver != null) { try { unregisterReceiver(powerReceiver); } catch (Exception ignored) {} }
        if (dot != null) { try { windowManager.removeView(dot); } catch (Exception ignored) {} }
        super.onDestroy();
    }
